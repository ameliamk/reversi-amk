  ...
  for (let row = 0; row < 8; row++) {
    for (let column = 0; column < 8; column++) {
      ...
      if (old_board[row][column] !== board[row][column]) {
        ...
        $("#" + row + "_" + column).html(
          '<img class="img-fluid" src="assets/images/' + graphic + "?time=" + t + '" alt="' + altTag + '"/>'
        );
...
socket.on("game_over", (payload) => {
  ...
  let nodeD = $(
    '<a href="lobby.html?username=' + username + '" class="btn btn-lg btn-success" role="button">Return to lobby</a>'
  );
...
$(() => {
  ...
  $("#quit").html(
    '<a href="lobby.html?username=' + username + '" class="btn btn-lg btn-danger" role="button">Quit</a>'
  );
